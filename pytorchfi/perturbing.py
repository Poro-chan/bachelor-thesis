...
testset2 = torchvision.datasets.MNIST(root='./data', download=True, train=True, transform=transform)
...
model = torch.hub.load('pytorch/vision:v0.10.0', 'mobilenet_v2', pretrained=True) # densenet121, googlenet, inception_v3, mobilenet_v2, resnet18 (squeezenet1_0, squeezenet1_1) -> excluded
...
while epsilon < 0.55:
    ...
    p_custom = func.custom_func(model, batch_size, epsilon, 5, input_shape=[3,224,224], layer_types=[torch.nn.Conv2d], use_cuda=torch.cuda.is_available())
    perturbed_model = p_custom.declare_neuron_fault_injection(function=p_custom.multiply_value)
    perturbed_model.eval()
    print("Multiplication of all values {} for mnist:".format(epsilon))
    func.evaluate(cifar100loader, perturbed_model, device)
...
while epsilon < 0.55:
    ...
    perturbed_model = func.perturb_weights(pfi, epsilon)
    perturbed_model.eval()
    print("Multiplication of all values {} for mnist:".format(epsilon))
    func.evaluate(cifar100loader, perturbed_model, device)
    if epsilon > 0.09: epsilon += 0.1
...
while index < 31 :
        pfi = FaultInjection(model, batch_size, input_shape=[3,224,224], layer_types=[torch.nn.Conv2d], use_cuda=torch.cuda.is_available())
        #print("total layers {}".format(pfi.get_total_layers())) #57
        perturbed_model2 = func.perturb_weights(pfi, index, func.flip_single_bit)
...
while index < 31 :
        ...
        perturbed_model2 = p_custom2.declare_neuron_fault_injection(function=p_custom2.single_bit_flip_layer)
        perturbed_model2.eval()
        print("Accuracy of the model with single bitflip, flipped {} for mnist:".format(index))
        func.evaluate(cifar10loader, perturbed_model2, device)
        index = index + 1
